...
module.exports = {
  ...
  async getTitleByTypeId(model) {
    if (model.state !== '0') {
      ...
      return await dbUtils.query(_sql,
        [articleDB.ARTICLE_RELEASE_TIME,
  ...
  async getAllArticleType() {
    ...
    // return result;
    let _sql = 'SELECT ??,??,??,??,??,?? AS type_id FROM ?? AS t1 LEFT JOIN ?? AS t2 ON ?? = ?? ORDER BY ??';
    return await dbUtils.query(_sql,
      [
        't1.' + articleDB.ARTICLE_ID,
        articleDB.ARTICLE_TITLE,
        articleDB.ARTICLE_RELEASE_TIME,
        articleDB.ARTICLE_EDIT_TIME,
        't2.' + articleTypeDB.ARTICLE_TYPE,
        't2.'+articleTypeDB.ARTICLE_TYPE_ID,
        
        articleDB.ARTICLE_TABLE,
        articleTypeDB.ARTICLE_TYPE_TABLE,
        't1.' + articleDB.ARTICLE_TYPE,
        't2.' + articleTypeDB.ARTICLE_TYPE_ID,
        't2.' + articleTypeDB.ARTICLE_TYPE,
      ]);
  },
