...
export default class TimeLine extends Component {
  ...
  render() {
    ...
    let timeline = article.filter(_ => _.typeId === parseInt(match.params.typeId))[0] || { content: [], type: '', typeId: '' };
    return (
      <div className="timeline-main">
        <Timeline>
          <Timeline.Item dot={<Icon type="file-text" style={{ fontSize: '16px' }}/>}>{timeline.type}</Timeline.Item>
          {
            timeline.content.map((item,index)=>{
              return (
                <Timeline.Item key={index}>
                  <Card className="timeline-content" bodered={false} hoverable>
                    <Icon type="calendar"/>{moment(item.releaseTime).format('YYYY-MM-DD HH:MM:SS')}
                    <hr/>
